...
def setup_groups_and_permissions(sender, **kwargs):
    ...
    staff_group, _ = Group.objects.get_or_create(name="Administradores")
...
class IsStudent(permissions.IsAuthenticatedOrReadOnly):
    def has_permission(self, request, view):
        return bool(request.user.groups.filter(name="Alumnos"))
